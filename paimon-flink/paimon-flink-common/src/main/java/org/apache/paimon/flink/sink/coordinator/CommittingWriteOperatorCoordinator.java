...
public class CommittingWriteOperatorCoordinator implements OperatorCoordinator {
    ...
    private final Map<Integer, SubtaskGateway> ackTargets = new ConcurrentHashMap<>();
    ...
    public CommittingWriteOperatorCoordinator(
        ...
        this(context, committerFactory, streamingCheckpointEnabled, initialCommitUser, null);
    }
                ...
                () -> {
                    if (state == State.RESTORING) {
                        ...
                        for (int i = 0; i < parallelism; i++) {
                            sendCommitSuccess(i, restoredCheckpointId);
                        }
                    } else {
        ...
        runInEventLoop(
                () -> {
                    ...
                    if (globalFinalizationCompleted) {
                        for (int i = 0; i < parallelism; i++) {
                            sendCommitSuccess(i, checkpointId);
                        }
                        return;
                    }
    ...
    public void executionAttemptReady(int subtask, int attemptNumber, SubtaskGateway gateway) {
        checkState(
                ...
                "Invalid ACK gateway");
        ackTargets.put(subtask, gateway);
        runInEventLoop(
                () -> {
                    if (ackTargets.get(subtask) == gateway) {
                        sendCommitSuccess(subtask, restoredCheckpointId);
                    }
                },
                "replaying commit success to writer %d",
                subtask);
    }
    ...
    private void handleRestoredCommittableEvent(int subtask, RestoredCommittableEvent event)
        ...
        if (terminalCoveredBy[subtask] >= 0) {
            ...
            sendCommitSuccess(subtask, event.getRestoredCheckpointId());
            return;
        }
        if (state == State.RESTORING) {
            checkState(
    ...
    private void retireAndPromote(long checkpointId) throws Exception {
        ...
        if (allTerminal()) {
            ...
            // Earlier attempts may have missed their first ACK; release every current target.
            for (int i = 0; i < parallelism; i++) {
                sendCommitSuccess(i, checkpointId);
            }
            return;
    ...
    private void sendCommitSuccess(int subtask, long replayCheckpoint) {
        if (fatalFailure != null || terminalCoveredBy[subtask] < 0) {
            return;
        }
        if (allTerminal() && !globalFinalizationCompleted) {
            return;
        }
        SubtaskGateway gateway = ackTargets.get(subtask);
        if (gateway != null) {
            // A later restored snapshot also covers the already committed terminal tail.
            // This allows replay to match empty terminal markers retained in that snapshot.
            long coverage = Math.max(terminalCoveredBy[subtask], replayCheckpoint);
            gateway.sendEvent(
                    new CommitSucceededEvent(
    ...
    public static class Provider extends RecreateOnResetOperatorCoordinator.Provider {
        ...
        public Provider(
                ...
                boolean streamingCheckpointEnabled,
                String initialCommitUser) {
            this(operatorId, committerFactory, streamingCheckpointEnabled, initialCommitUser, null);
        }
