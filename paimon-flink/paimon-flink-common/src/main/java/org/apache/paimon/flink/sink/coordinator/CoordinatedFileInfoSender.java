...
public class CoordinatedFileInfoSender {
    ...
    public void notifyEndInput() {
        if (!this.streamingCheckpointEnabled) {
            sendToCoordinator(CommitterCoordinator.END_INPUT_CHECKPOINT_ID);
            return;
        }
        try {
            gateway.sendOperatorEventToCoordinator(
                    operatorID, new SerializedValue<>(new EndInputEvent()));
        } catch (IOException e) {
            throw new RuntimeException("Failed to send EndInput event to Coordinator", e);
        }
    }
